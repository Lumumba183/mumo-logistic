          ...
          <div className="flex justify-between items-center h-16 lg:h-20">
            ...
            <a href="#" className="flex items-center gap-3 group" onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
              <img 
                ...
                className="h-10 w-10 sm:h-12 sm:w-12 rounded-lg object-cover shadow-md group-hover:shadow-lg transition-shadow"
              />
              <div className="hidden sm:block">
                <span className={`font-display font-bold text-lg lg:text-xl transition-colors ${isScrolled ? 'text-navy' : 'text-white'}`}>
                  MumoFreight
                </span>
                <span className={`block text-[10px] lg:text-xs font-medium tracking-wider uppercase transition-colors ${isScrolled ? 'text-gray-500' : 'text-white/70'}`}>
            ...
            <div className="hidden lg:flex items-center gap-8">
              {navLinks.map((link) => (
                <button
                  ...
                  className={`font-medium text-sm transition-colors hover:text-accent ${isScrolled ? 'text-gray-700' : 'text-white/90'}`}
                >
                  {link.label}
                </button>
              ))}
            ...
            <div className="hidden lg:block">
              <button
                ...
                className="mumo-btn-primary text-sm"
              >
                Get a Quote
              </button>
            </div>
            ...
            <div className="mt-12 flex flex-col gap-4">
              {navLinks.map((link) => (
                <button
                  key={link.href}
                  onClick={() => scrollToSection(link.href)}
                  className="text-left text-navy font-medium text-lg py-2 border-b border-gray-100 hover:text-accent transition-colors"
                >
                  {link.label}
                </button>
              ))}
              <button
                onClick={() => scrollToSection('#quote')}
                className="mumo-btn-primary mt-4"
              >
                Get a Quote
              </button>
            </div>
